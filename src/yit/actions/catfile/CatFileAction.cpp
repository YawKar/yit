...
void CatFileAction::cat_file(const std::string& object_name,
                             bool pretty_print) {
  if (object_name.length() != 40) {
    throw std::runtime_error(
        fmt::format("Object name should be 40 characters long but was {}",
                    object_name.length()));
  }
  ...
  auto object = repo.get_repo_file(
      {"objects", object_name.substr(0, 2), object_name.substr(2)}, false);
  if (!fs::exists(object)) {
    throw std::runtime_error(
        fmt::format("Object was not found, name: {}", object_name));
  }
  if (!pretty_print) {
